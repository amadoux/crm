...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'birthDate' | 'entryDate' | 'releaseDate' | 'hireDate'> & {
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...employee,
      birthDate: employee.birthDate?.toJSON() ?? null,
      entryDate: employee.entryDate?.toJSON() ?? null,
      releaseDate: employee.releaseDate?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...
      entryDate: restEmployee.entryDate ? dayjs(restEmployee.entryDate) : undefined,
      releaseDate: restEmployee.releaseDate ? dayjs(restEmployee.releaseDate) : undefined,
